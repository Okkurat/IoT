const ctx = document.getElementById('myChart');
      ...
      function fetch_data(){
        ...
        if(cb_humidity.checked)    path += "rh=1&"
        if(cb_set_point.checked)   path += "setpoint=1&"
        if(cb_pressure.checked)    path += "pressure=1&"
        if(cb_speed.checked)       path += "speed=1&"
        if(cb_temperature.checked) path += "temperature=1&"
        if(cb_co2.checked)         path += "co2=1&"
